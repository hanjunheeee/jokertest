 ...
 * (state.nightTurnRoles)이 정하고, 그 순서를 어디까지 밟을지는 canonical 역할 턴이 상한으로
 * 정하며(useInGameNightTurnAnnouncement), 칸 하나의 노출 시간만 프런트의 고정 타이머가 만든다.
 *
...
/**
 * 릴 한 칸의 **최소 노출 시간**(ms) — 안내 한 장이 저절로 닫히는 시간이자, 커서가 다음 칸으로
 * 넘어갈 수 있게 되기까지 기다리는 시간이다.
 *
 * 이 값만으로 릴이 흐르지는 않는다. 커서는 canonical 역할 턴을 상한으로만 전진하므로
 * (computeInGameNightTurnReelBarrier), 보유자가 살아있는 역할의 칸은 이 시간이 지나도 그 역할의
 * 제출이 서버에 닿기 전까지 넘어가지 않는다. 반대로 canonical에 커서를 곧장 맞추지도 않는다 —
 * 상한이 여러 칸 앞으로 뛴 밤(죽은 역할이 낀 밤)에 중간 칸이 한 프레임 만에 지워져 누출이
 * 타이밍 의존으로 되살아나기 때문이다. 그래서 모든 칸이 어느 창에서나 똑같이 이 시간을 갖는다.
 */
