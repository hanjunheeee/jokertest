...
import { useInGameNightPrivateResult } from "./useInGameNightPrivateResult.js"
 ...
 *   3. 개인 조사 결과(nightPrivateResult — GUARD·WITCH_HUNTER 본인에게만 온 night_action_result)
 *   4. DAY/NIGHT 진입 연출("낮이 되었습니다" / "밤이 되었습니다")
 *   5. 밤 역할 턴 안내("광대/의사/경호원/마녀사냥꾼의 시간입니다")
 *   6. 그 아래 게임 표면의 조작
 *
 * 2번(killReveal)은 1번이 떠 있는 동안 보류된다(hold) — 자기 역할 확인이 사망 연출보다
 * 먼저다. killReveal.open이 true인 동안(=큐에서 승격된 항목이 실제로 재생 중인 동안) 3·4·5번이
 * 자동으로 그 뒤로 밀린다 — 진입 연출은 기다리는 동안 소비되지 않으므로, 사망 연출 큐가 모두
 * 끝난 뒤에 반드시 뜬다(같은 밤에 여러 희생자가 있어도 큐가 다 빌 때까지 끊기지 않는다).
 *
 * 3번은 확인 버튼을 누를 때까지 소비되지 않는다(결과가 canonical store에 남아 있고, 지워지는
 * 시점은 확인·NIGHT 재진입·gameId 변경뿐이다) — 그래서 4·5번이 그 뒤로 밀린다.
 *
...
export function useInGameOverlayStack() {
  ...
  const nightPrivateResult = useInGameNightPrivateResult({
    hold: roleReveal.open || killReveal.open,
  })

  // 1·2·3번이 하나라도 떠 있으면 4번은 대기한다(대기 중에도 소비되지 않는다).
  ...
  // 5번은 4번이 완전히 끝나야(대기 중인 것도 없어야) 시작한다 — "밤이 되었습니다"를 닫기
  ...
  return {
    ...
    // 개인 조사 결과도 확인 전까지 화면을 덮으므로 함께 잠근다.
